...
describe('Plan Command - Amend Functionality', () => {
  ...
  describe('Project Resolution for Amend', () => {
    beforeEach(() => {
      ...
      const projectDir = path.join(tempDir, '3-my-project');
      fs.mkdirSync(projectDir);
      fs.mkdirSync(path.join(projectDir, 'plans'));
      fs.writeFileSync(
        path.join(projectDir, 'plans', '1-first-task.md'),
        '# Task: First Task\n'
      );
      fs.writeFileSync(
        path.join(projectDir, 'plans', '2-second-task.md'),
        '# Task: Second Task\n'
      );
    });
    ...
    it('should resolve project by full folder name for amend', () => {
      const result = resolveProjectIdentifier(tempDir, '3-my-project');
      expect(result).toBe(path.join(tempDir, '3-my-project'));
    });
  ...
  describe('Numeric Project Resolution for Amend', () => {
    beforeEach(() => {
      const projectDir = path.join(tempDir, '42-important-project');
      fs.mkdirSync(projectDir);
      fs.mkdirSync(path.join(projectDir, 'plans'));
      fs.writeFileSync(
        path.join(projectDir, 'plans', '1-initial-task.md'),
        '# Task: Initial Task\n'
      );
    });

    it('should resolve project by numeric prefix for amend', () => {
      const result = resolveProjectIdentifier(tempDir, '42');
      expect(result).toBe(path.join(tempDir, '42-important-project'));
    });

    it('should resolve project by full numeric folder name for amend', () => {
      const result = resolveProjectIdentifier(tempDir, '42-important-project');
      expect(result).toBe(path.join(tempDir, '42-important-project'));
    });
  });
  ...
  describe('Existing Project State Loading', () => {
    it('should load pending tasks', () => {
      const projectDir = path.join(tempDir, '1-test-project');
      fs.mkdirSync(projectDir);
      fs.mkdirSync(path.join(projectDir, 'plans'));
      fs.writeFileSync(
        path.join(projectDir, 'plans', '1-task-one.md'),
        '# Task: Task One\n'
      );
      fs.writeFileSync(
        path.join(projectDir, 'plans', '2-task-two.md'),
        '# Task: Task Two\n'
      );
    ...
    it('should load completed tasks', () => {
      ...
      fs.writeFileSync(
        path.join(projectDir, 'outcomes', '1-task-one.md'),
    ...
    it('should load mixed task statuses', () => {
      ...
      fs.writeFileSync(
        path.join(projectDir, 'plans', '3-task-three.md'),
        '# Task: Task Three\n'
      );
      fs.writeFileSync(
        path.join(projectDir, 'outcomes', '1-task-one.md'),
      ...
      fs.writeFileSync(
        path.join(projectDir, 'outcomes', '2-task-two.md'),
        'Task failed.\n\n<promise>FAILED</promise>'
      );
  ...
  describe('Task Name Extraction', () => {
    it('should extract task name from plan file', () => {
      const name = extractTaskNameFromPlanFile('1-fix-login-bug.md');
      expect(name).toBe('fix-login-bug');
    });

    it('should extract task name from plan file with multiple hyphens', () => {
      const name = extractTaskNameFromPlanFile('2-add-user-registration-flow.md');
      expect(name).toBe('add-user-registration-flow');
    });

    it('should handle 2-digit task numbers', () => {
      const name = extractTaskNameFromPlanFile('1-task.md');
  ...
  describe('Amend Prompt Generation', () => {
    it('should return systemPrompt and userMessage', () => {
      const params: AmendPromptParams = {
        ...
        existingTasks: [
          { id: '1', planFile: 'plans/1-first.md', status: 'completed', taskName: 'first' },
          { id: '2', planFile: 'plans/2-second.md', status: 'pending', taskName: 'second' },
        ],
        nextTaskNumber: 3,
    ...
    it('should include failed task status with MODIFIABLE indicator in system prompt', () => {
      const params: AmendPromptParams = {
        projectPath: '/test/project',
        existingTasks: [
          { id: '1', planFile: 'plans/1-first.md', status: 'failed', taskName: 'first' },
        ],
        nextTaskNumber: 2,
        newTaskDescription: 'New tasks',
      };
      ...
      expect(systemPrompt).toContain('Task 1: first [FAILED] [MODIFIABLE]');
    });
    ...
    it('should list protected tasks separately', () => {
      const params: AmendPromptParams = {
        ...
        existingTasks: [
          { id: '1', planFile: 'plans/1-first.md', status: 'completed', taskName: 'first' },
          { id: '2', planFile: 'plans/2-second.md', status: 'completed', taskName: 'second' },
          { id: '3', planFile: 'plans/3-third.md', status: 'pending', taskName: 'third' },
        ],
        nextTaskNumber: 4,
        newTaskDescription: 'New tasks',
      };
    ...
    it('should list modifiable tasks separately', () => {
      const params: AmendPromptParams = {
        ...
        existingTasks: [
          { id: '1', planFile: 'plans/1-first.md', status: 'completed', taskName: 'first' },
          { id: '2', planFile: 'plans/2-second.md', status: 'pending', taskName: 'second' },
          { id: '3', planFile: 'plans/3-third.md', status: 'failed', taskName: 'third' },
        ],
        nextTaskNumber: 4,
        newTaskDescription: 'New tasks',
      };
    ...
    it('should show (none) when there are no protected tasks', () => {
      const params: AmendPromptParams = {
        projectPath: '/test/project',
        existingTasks: [
          { id: '1', planFile: 'plans/1-first.md', status: 'pending', taskName: 'first' },
        ],
        nextTaskNumber: 2,
        newTaskDescription: 'New tasks',
      };
    ...
    it('should handle mixed task statuses correctly', () => {
      const params: AmendPromptParams = {
        projectPath: '/test/project',
        existingTasks: [
          { id: '1', planFile: 'plans/1-first.md', status: 'completed', taskName: 'setup' },
          { id: '2', planFile: 'plans/2-second.md', status: 'completed', taskName: 'database' },
          { id: '3', planFile: 'plans/3-third.md', status: 'failed', taskName: 'api' },
          { id: '4', planFile: 'plans/4-fourth.md', status: 'pending', taskName: 'tests' },
        ],
        nextTaskNumber: 5,
        newTaskDescription: 'New tasks',
      };
      ...
      // Check summary list
      expect(systemPrompt).toContain('Task 1: setup [COMPLETED] [PROTECTED]');
      expect(systemPrompt).toContain('Task 2: database [COMPLETED] [PROTECTED]');
      expect(systemPrompt).toContain('Task 3: api [FAILED] [MODIFIABLE]');
    ...
    it('should include correct plans directory path', () => {
      const params: AmendPromptParams = {
        ...
        existingTasks: [
          { id: '1', planFile: 'plans/1-task.md', status: 'pending', taskName: 'task' },
        ],
        nextTaskNumber: 2,
        newTaskDescription: 'New tasks',
      };
    ...
    it('should include outcome file paths for completed tasks in task summary', () => {
      const params: AmendPromptParams = {
        projectPath: '/test/project',
        existingTasks: [
          { id: '1', planFile: 'plans/1-setup.md', status: 'completed', taskName: 'setup' },
          { id: '2', planFile: 'plans/2-database.md', status: 'completed', taskName: 'database' },
        ],
        nextTaskNumber: 3,
      ...
      expect(systemPrompt).toContain('Outcome: /test/project/outcomes/1-setup.md');
      expect(systemPrompt).toContain('Outcome: /test/project/outcomes/2-database.md');
    });
    ...
    it('should not include outcome file paths for non-completed tasks', () => {
      const params: AmendPromptParams = {
        projectPath: '/test/project',
        existingTasks: [
          { id: '1', planFile: 'plans/1-pending-task.md', status: 'pending', taskName: 'pending-task' },
          { id: '2', planFile: 'plans/2-failed-task.md', status: 'failed', taskName: 'failed-task' },
        ],
        nextTaskNumber: 3,
        newTaskDescription: 'New tasks',
      };
      ...
      expect(systemPrompt).not.toContain('Outcome: /test/project/outcomes/1-pending-task.md');
      expect(systemPrompt).not.toContain('Outcome: /test/project/outcomes/2-failed-task.md');
    });
  ...
  describe('Existing Project Detection Without Amend Flag', () => {
    ...
    it('should find existing project by numeric ID', () => {
      fs.mkdirSync(path.join(tempDir, '1-existing-project'));
    ...
    it('should find existing project by full folder name', () => {
      fs.mkdirSync(path.join(tempDir, '1-existing-project'));
      const result = resolveProjectIdentifier(tempDir, '1-existing-project');
      expect(result).not.toBeNull();
    });
