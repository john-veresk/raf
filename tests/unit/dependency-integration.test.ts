...
describe('Dependency Integration Flow', () => {
  ...
  describe('Complete dependency chain simulation', () => {
    ...
    beforeEach(() => {
      ...
      fs.writeFileSync(
        path.join(projectPath, 'plans', '1-setup.md'),
        '# Task: Setup\n\n## Objective\nSetup the project infrastructure.\n\n## Requirements\n- Initialize project'
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '2-core.md'),
        '# Task: Core\n\n## Objective\nBuild core functionality.\n\n## Dependencies\n1\n\n## Requirements\n- Build core'
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '3-api.md'),
        '# Task: API\n\n## Objective\nImplement API layer.\n\n## Dependencies\n1, 2\n\n## Requirements\n- Create API'
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '4-tests.md'),
        '# Task: Tests\n\n## Objective\nWrite tests.\n\n## Dependencies\n3\n\n## Requirements\n- Write tests'
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '5-docs.md'),
        '# Task: Docs\n\n## Objective\nWrite documentation.\n\n## Dependencies\n3\n\n## Requirements\n- Write docs'
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '6-deploy.md'),
        '# Task: Deploy\n\n## Objective\nDeploy the project.\n\n## Dependencies\n4, 5\n\n## Requirements\n- Deploy'
      );
    });
    ...
    it('should correctly derive initial state with all tasks pending', () => {
      ...
      expect(state.tasks[3]?.dependencies).toEqual(['3']);
      expect(state.tasks[4]?.dependencies).toEqual(['3']);
      expect(state.tasks[5]?.dependencies).toEqual(['4', '5']);
    });
    ...
    it('should cascade blocks when task 001 fails', () => {
      // Task 001 fails
      fs.writeFileSync(
        path.join(projectPath, 'outcomes', '1-setup.md'),
    ...
    it('should cascade blocks when task 002 fails (001 completed)', () => {
      ...
      // Task 002 fails
      fs.writeFileSync(
        path.join(projectPath, 'outcomes', '2-core.md'),
        '# Outcome\n\nCore failed.\n\n<promise>FAILED</promise>'
      );
    ...
    it('should cascade blocks when task 003 fails (001, 002 completed)', () => {
      ...
      // Task 003 fails
      fs.writeFileSync(
        path.join(projectPath, 'outcomes', '3-api.md'),
        '<promise>FAILED</promise>'
      );
    ...
    it('should handle partial failure in parallel branches', () => {
      ...
      fs.writeFileSync(
        path.join(projectPath, 'outcomes', '4-tests.md'),
        '<promise>COMPLETE</promise>'
      );
      fs.writeFileSync(
        path.join(projectPath, 'outcomes', '5-docs.md'),
        '<promise>FAILED</promise>'
      );
    ...
    it('should mark project complete when all tasks succeed', () => {
      // All tasks complete
      for (const taskNum of ['1', '2', '3', '4', '5', '6']) {
  ...
  describe('getNextExecutableTask with complex dependency states', () => {
    it('should skip blocked tasks and find next available pending task', () => {
      ...
      fs.writeFileSync(
        path.join(projectPath, 'plans', '1-first.md'),
        '# Task: First'
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '2-second.md'),
        '# Task: Second\n\n## Dependencies\n1'
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '3-third.md'),
        '# Task: Third'
      );

      // 1 fails
      fs.writeFileSync(
        path.join(projectPath, 'outcomes', '1-first.md'),
        '<promise>FAILED</promise>'
      );
    ...
    it('should return failed task for retry when no pending tasks exist', () => {
      ...
      // Should return 1 for retry (2 is blocked)
      expect(nextTask?.id).toBe('1');
    ...
    it('should return null when all tasks are blocked with no failed tasks to retry', () => {
      ...
      fs.writeFileSync(
        path.join(projectPath, 'outcomes', '2-second.md'),
  ...
  describe('Execution prompt with dependency context', () => {
    it('should include dependency outcomes in execution prompt', () => {
      ...
      fs.writeFileSync(
        path.join(projectPath, 'plans', '2-build.md'),
        '# Task: Build\n\n## Objective\nBuild the project.\n\n## Dependencies\n1'
      );
      ...
      const state = deriveProjectState(projectPath);
      const task002 = state.tasks.find((t) => t.id === '2');

      expect(task002).toBeDefined();
      expect(task002?.dependencies).toEqual(['1']);

      // Build execution prompt for task 002
      const prompt = getExecutionPrompt({
        projectPath,
        planPath: path.join(projectPath, 'plans', '2-build.md'),
        taskId: '2',
        taskNumber: 2,
        totalTasks: 2,
        ...
        dependencyIds: ['1'],
        dependencyOutcomes: [{ taskId: '1', content: outcome001 }],
      });
    ...
    it('should include multiple dependency outcomes', () => {
      ...
      fs.writeFileSync(path.join(projectPath, 'outcomes', '1-setup.md'), outcome001);
      fs.writeFileSync(path.join(projectPath, 'outcomes', '2-core.md'), outcome002);

      const prompt = getExecutionPrompt({
        projectPath,
        planPath: path.join(projectPath, 'plans', '3-api.md'),
        taskId: '3',
        taskNumber: 3,
        totalTasks: 3,
        previousOutcomes: [
...
<promise>BLOCKED</promise>`;
  ...
  describe('Edge cases', () => {
    it('should handle circular dependency detection (prevented by lower-ID rule)', () => {
      ...
      fs.writeFileSync(
        path.join(projectPath, 'plans', '1-first.md'),
        '# Task: First\n\n## Dependencies\n2' // Invalid: depends on higher-numbered task
      );
      fs.writeFileSync(
        path.join(projectPath, 'plans', '2-second.md'),
        '# Task: Second'
