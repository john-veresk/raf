...
describe('stashChanges', () => {
  ...
  it('should stash changes with descriptive name', () => {
    ...
    const result = stashChanges('raf-1-task-3-failed');
    expect(result).toBe(true);
    expect(mockExecSync).toHaveBeenCalledWith(
      'git stash push -m "raf-1-task-3-failed"',
      expect.any(Object)
    );
  });
  ...
  it('should use correct stash name format for RAF failures', () => {
    ...
    const result = stashChanges('raf-2-task-5-failed');
    expect(result).toBe(true);
    expect(mockExecSync).toHaveBeenCalledWith(
      'git stash push -m "raf-2-task-5-failed"',
      expect.any(Object)
    );
  });
});
